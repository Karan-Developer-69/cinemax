...
const AdminUsersTab = () => {
    ...
    return (
        <div className="bg-secondary-bg/50 rounded-xl border border-border-color p-6 overflow-x-auto">
            <h2 className="text-2xl font-bold mb-6 text-text-primary">Registered Users</h2>
            <table className="w-full text-left border-collapse min-w-[600px]">
                ...
                <tbody>
                    {users.map(u => (
                        <tr key={u._id} className="border-b border-border-color hover:bg-text-secondary/5 transition-colors">
                            <td className="py-4 px-4 font-medium text-text-primary">{u.username}</td>
                            <td className="py-4 px-4 text-text-secondary">{u.email}</td>
                            <td className="py-4 px-4">
                                <span className={`px-2 py-1 rounded text-xs font-bold ${u.role === 'admin' ? 'bg-accent-gold/20 text-accent-gold' : 'bg-secondary-bg text-text-primary'}`}>
                                    {u.role.toUpperCase()}
                                </span>
                            </td>
                            <td className="py-4 px-4">
                            ...
                            <td className="py-4 px-4 flex items-center justify-end gap-3">
                                <button
                                    ...
                                    className="p-2 glass-light rounded-full hover:bg-border-color text-blue-500 transition-colors"
                                >
                                ...
                                <button
                                    ...
                                    className="p-2 glass-light rounded-full hover:bg-border-color text-orange-500 transition-colors"
                                >
                                    <Ban size={16} />
                                </button>
                                <button
                    ...
                    {users.length === 0 && (
                        <tr><td colSpan="5" className="py-8 text-center text-text-secondary/50">No users found.</td></tr>
                    )}
                </tbody>
            </table>
        </div>
